...
/** @file error.hpp Error values, registers, handlers, and systems. */
...
{
    /** @brief Coarse operational category attached to an Error. */
    ...
    /** @brief Owning error value used by MEF runtime components. */
    class Error
    {
    public:
    ...
    /** @brief Recoverable operation result carrying an MEF Error on failure. */
    template<typename Type>
    using Result = std::expected<Type, Error>;

    /** @brief Recoverable operation result without a success value. */
    using OperationResult = Result<void>;

    /** @brief Concrete error provider satisfying the replaceable MCF model. */
    struct Model
    {
        using Error = vosp::error::Error;

        template<typename Type>
        using Result = vosp::error::Result<Type>;

        using OperationResult = vosp::error::OperationResult;

        [[nodiscard]] static Error make_error(
            std::uint32_t code,
            std::string message)
        {
            return Error{Category::NONE, code, std::move(message)};
        }
    };
    ...
    {
        return std::format("[{}:{}] {}", category_name(error.category()),
    ...
    /** @brief Invokes a callable and translates exceptions into an Error result. */
