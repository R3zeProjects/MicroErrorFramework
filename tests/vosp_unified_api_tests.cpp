...
namespace
{
    ...
    bool test_sink_and_logger_policies()
    {
        ...
        Logger<logger_policy::AcceptAll,
               ...
               logger_policy::MinimalMetadata> parallel{buffered};
        ...
        return check(parallel.warning(buffered_error),
               ...
               check(buffered_output.str().empty(),
                     "buffered sink delays output") &&
...
int main()
{
    return test_register_and_system_policies() &&
           test_sink_and_logger_policies() &&
           test_async_system_policy() ? 0 : 1;
}
